...
from icg_benchmark.third_party.ConvOnet.dataset import DatasetPointsOccGeo
...
def evaluate_network(
    ...
    # create data loaders
    test_set, test_loader, size = create_test_loader(dataset_folder, with_table=with_table)
    ...
    with torch.no_grad():
        for idx, (data, gt_mesh) in tqdm.tqdm(enumerate(test_loader), total=len(test_loader), dynamic_ncols=True):
            data_in, points_occ, occ = data
            pc_in = data_in["pc"].float().to(device)
            points_occ = points_occ.float().to(device)
            occ = occ.float().to(device)
            gt_mesh = gt_mesh[0]
            # gt_mesh.vertices = gt_mesh.vertices / test_set.size - 0.5

            pred_mesh = predict_mesh(
                generator, pc_in, data_in["normals"].float().to(device), data_in["colors"].float().to(device)
            )
...
def create_test_loader(root_raw, num_point_occ=100000, with_table=False):
    # load the dataset
    ...
    dataset = DatasetPointsOccGeo(None, root_raw, num_point_occ=num_point_occ, normalize=False, with_table=with_table)
...
def predict_mesh(generator, pc_input, pc_normals, pc_colors):
    pred_mesh, _ = generator.generate_mesh({"inputs": pc_input, "normals": pc_normals, "colors": pc_colors})
    return pred_mesh
