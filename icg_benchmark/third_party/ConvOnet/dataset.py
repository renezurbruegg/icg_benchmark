...
from icg_benchmark.third_party.ConvOnet.implicit import (
    check_mesh_contains,
...
class DatasetPointsOccGeo(torch.utils.data.Dataset):
    def __init__(self, root, raw_root, num_point_occ=2048, augment=False, normalize=True, with_table=False):
        self.num_point_occ = num_point_occ
        self.raw_root = raw_root
        self.num_th = 32
        self.df = read_df(raw_root)
    ...
    def __getitem__(self, i):
        ...
        mask = np_file["pc"][:, -1] > 0.055 if not self.with_table else np_file["pc"][:, -1] > 0.0
...
class DatasetVoxelOccGeoROI(torch.utils.data.Dataset):
    def __init__(self, root, raw_root, num_point_occ=2048, ROI_scale=0.3, uniform=True):
        self.root = root
        self.num_point_occ = num_point_occ
        self.raw_root = raw_root
        self.num_th = 32
    ...
    def __getitem__(self, i):
        ...
        for grasp_idx in self.scene_dict[scene_id]:
            ori = Rotation.from_quat(self.df.loc[grasp_idx, "qx":"qw"].to_numpy(np.single))
        ...
        num_occ_point_per_grasp = self.num_point_occ * np.array(width_list, dtype=float)
        ...
        occ_points_list = []
        for num, pos, width, ori in zip(num_occ_point_per_grasp, pos_list, width_list, ori_list):
    ...
    def sample_occ(self, scene_id, num_point):
        ...
        mesh_pose_list = np.load(mesh_pose_list_path, allow_pickle=True)["pc"]
        scene, mesh_list = get_scene_from_mesh_pose_list(mesh_pose_list, return_list=True)
        points, occ = sample_iou_points(
            mesh_list, scene.bounds, num_point, uniform=self.uniform, size=self.size, padding=0
        )
        return points, occ, scene
