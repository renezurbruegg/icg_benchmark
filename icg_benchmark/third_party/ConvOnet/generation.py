...
from icg_benchmark.third_party.ConvOnet.common import add_key, coord2index, make_3d_grid, normalize_coord

try:
    from icg_benchmark.third_party import libmcubes
    from icg_benchmark.third_party.libmise import MISE
    from icg_benchmark.third_party.libsimplify import simplify_mesh
...
class Generator3D(object):
    ...
    def generate_mesh(self, data, return_stats=True, **kwargs):
        ...
        if self.vol_bound is not None:
            ...
            with torch.no_grad():
                if not self.normalize:
                    c = self.model.encode_inputs(
                        coords=data["inputs"].squeeze(0),
                        colors=data["colors"].squeeze(0),
        ...
        # c = (c[0][0].unsqueeze(0), c[1][0].unsqueeze(0))
        self.c = c
    ...
    def generate_from_latent(self, c=None, stats_dict={}, **kwargs):
        ...
        if self.upsampling_steps == 0:
            ...
            while points.shape[0] != 0:
                ...
                # fix bottom parts
                if self.crop_output:
                    values[(pointsf[:, -1] < -0.4).cpu().numpy()] = -2
                    values[(pointsf[:, -1] > 0.45).cpu().numpy()] = -2
    ...
    def generate_mesh_sliding(self, data, return_stats=True):
        ...
        for i in trange(n_crop):
            ...
            if self.upsampling_steps == 0:
                ...
                pp = (
                    np.mgrid[bb_min[0] : bb_max[0] : t[0], bb_min[1] : bb_max[1] : t[1], bb_min[2] : bb_max[2] : t[2]]
                    .reshape(3, -1)
                    .T
                )
    ...
    def get_crop_bound(self, inputs):
        ...
        lb_query = (
            np.mgrid[lb[0] : ub[0] : query_crop_size, lb[1] : ub[1] : query_crop_size, lb[2] : ub[2] : query_crop_size]
            .reshape(3, -1)
            .T
        )
    ...
    def predict_crop_occ(self, pi, c, vol_bound=None, **kwargs):
        ...
        with torch.no_grad():
            occ_cur = self.model.decode_occ(pi_in, c, **kwargs).logits
        ...
        for pi in p_split:
            if self.input_type == "pointcloud_crop":
                if self.vol_bound is not None:  # sliding-window manner
                    ...
                    pi_in["p_n"] = p_n
                    with torch.no_grad():
                        occ_hat = self.model.decode_occ(pi_in, c, **kwargs).logits
                    occ_hats.append(occ_hat.squeeze(0).detach().cpu())
            else:
                pi = pi.unsqueeze(0).to(self.device)
                with torch.no_grad():
                    occ_hat = self.model.decode_occ(pi, c, **kwargs).logits
                occ_hats.append(occ_hat.squeeze(0).detach().cpu())
    ...
    def estimate_normals(self, vertices, c=None):
        ...
        for vi in vertices_split:
            ...
            vi.requires_grad_()
            occ_hat = self.model.decode_occ(vi, c).logits
            out = occ_hat.sum()
            out.backward()
            ni = -vi.grad
    ...
    def refine_mesh(self, mesh, occ_hat, c=None):
        ...
        for it_r in trange(self.refinement_step):
            ...
            face_value = torch.sigmoid(self.model.decode_occ(face_point.unsqueeze(0), c).logits)
