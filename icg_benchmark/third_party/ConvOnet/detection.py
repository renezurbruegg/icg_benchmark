...
class VGN(GraspPlannerModule[GigaTSDFObservation]):
    def __init__(
        self, model_path, model_type, best=True, force_detection=True, qual_th=0.9, out_th=0.5, visualize=False
    ): 
        super().__init__()
    ...
    def forward(self, state_cb, timer, scene_mesh=None, aff_kwargs={}, return_reconstruction=False):
        state = state_cb(timer)
        ...
        with timer["postproc"]:
            ...
            if self.visualize:
                colored_scene_mesh = visual.affordance_visual(
                    qual_vol, rot_vol.transpose(1, 2, 3, 0), scene_mesh, size, 40, **aff_kwargs
                )
            ...
            pose = grasps[0].pose.as_matrix()[None, ...]
...
def process(
    tsdf_vol, qual_vol, rot_vol, width_vol, gaussian_filter_sigma=1.0, min_width=1.33, max_width=9.33, out_th=0.5
):
