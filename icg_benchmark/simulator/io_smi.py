...
from icg_benchmark.simulator.perception import CameraIntrinsic, TSDFVolume
...
def write_mesh_data_with_corr(root, vertices, faces, vertice_normals, reasonable_index, corr):
    ...
    path = root / "meshes" / (scene_id + ".npz")
    np.savez_compressed(
        path, vertices=vertices, faces=faces, vertice_normals=vertice_normals, index=reasonable_index, corr=corr
    )
    return scene_id
